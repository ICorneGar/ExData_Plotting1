...
with(sub_data, {
                  plot(Global_active_power~Datetime, type="l", ylab="Global Active Power", xlab="") #plot (1,1)
                  plot(Voltage~Datetime, type="l", ylab="Voltage", xlab="") #plot (1,2)
                  plot(Sub_metering_1~Datetime, type="l", ylab="Global Active Power", xlab="") #plot (2,1)
                  lines(Sub_metering_2~Datetime,col='Red'); lines(Sub_metering_3~Datetime,col='Blue')
                  legend("topright", col=c("black", "red", "blue"), lty=1, lwd=2, bty="n",legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
                  plot(Global_reactive_power~Datetime, type="l", ylab="Global_Rective_Power",xlab="") #plot (2,2)
               }
     )
